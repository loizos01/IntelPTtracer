...
static int open_perf(size_t, struct hwt_cerror *,pid_t traceepid);

// Exposed Prototypes.
struct hwt_perf_ctx *hwt_perf_init_collector(struct hwt_perf_collector_config *, struct hwt_cerror *,pid_t traceepid);
 ...
 */
static int
open_perf(size_t aux_bufsize, struct hwt_cerror *err,pid_t traceepid) {
 ...
 */
struct hwt_perf_ctx *
hwt_perf_init_collector(struct hwt_perf_collector_config *tr_conf, struct hwt_cerror *err,pid_t traceepid)
{
    ...
    tr_ctx->perf_fd = open_perf(tr_conf->aux_bufsize, err,traceepid);
    if (tr_ctx->perf_fd == -1) {
        hwt_set_cerr(err, hwt_cerror_errno, errno);
        failing = true;
        goto clean;
